...
impl MetadataRecord {
    ...
    fn decode_heapam_record(
        ...
        match pg_version {
            ...
            PgMajorVersion::GB06 => {
                // openGauss v6: similar to PG17 but with v6 structs
                if decoded.xl_rmid == pg_constants::RM_HEAP_ID {
                    let info = decoded.xl_info & pg_constants::XLOG_HEAP_OPMASK;

                    if info == pg_constants::XLOG_HEAP_INSERT {
                        let xlrec = v6::XlHeapInsert::decode(buf);
                        assert_eq!(0, buf.remaining());
                        if (xlrec.flags & pg_constants::XLH_INSERT_ALL_VISIBLE_CLEARED) != 0 {
                            new_heap_blkno = Some(decoded.blocks[0].blkno);
                        }
                    } else if info == pg_constants::XLOG_HEAP_DELETE {
                        let xlrec = v6::XlHeapDelete::decode(buf);
                        if (xlrec.flags & pg_constants::XLH_DELETE_ALL_VISIBLE_CLEARED) != 0 {
                            new_heap_blkno = Some(decoded.blocks[0].blkno);
                        }
                    } else if info == pg_constants::XLOG_HEAP_UPDATE
                        || info == pg_constants::XLOG_HEAP_HOT_UPDATE
                    {
                        let xlrec = v6::XlHeapUpdate::decode(buf);
                        // the size of tuple data is inferred from the size of the record.
                        // we can't validate the remaining number of bytes without parsing
                        // the tuple data.
                        if (xlrec.flags & pg_constants::XLH_UPDATE_OLD_ALL_VISIBLE_CLEARED) != 0 {
                        ...
                        let xlrec = v6::XlHeapLock::decode(buf);
                        if (xlrec.flags & pg_constants::XLH_LOCK_ALL_FROZEN_CLEARED) != 0 {
                            old_heap_blkno = Some(decoded.blocks[0].blkno);
                            flags = pg_constants::VISIBILITYMAP_ALL_FROZEN;
                        }
                    ...
                    let info = decoded.xl_info & pg_constants::XLOG_HEAP_OPMASK;
                    if info == pg_constants::XLOG_HEAP2_MULTI_INSERT {
                        let xlrec = v6::XlHeapMultiInsert::decode(buf);
                        ...
                        let xlrec = v6::XlHeapLockUpdated::decode(buf);
                        if (xlrec.flags & pg_constants::XLH_LOCK_ALL_FROZEN_CLEARED) != 0 {
                            old_heap_blkno = Some(decoded.blocks[0].blkno);
                            flags = pg_constants::VISIBILITYMAP_ALL_FROZEN;
                        }
    ...
    fn decode_neonmgr_record(
        ...
        match pg_version {
            PgMajorVersion::PG16 | PgMajorVersion::PG17 | PgMajorVersion::GB06 => {
                let info = decoded.xl_info & pg_constants::XLOG_HEAP_OPMASK;
    ...
    fn decode_dbase_record(
        ...
        match pg_version {
            ...
            PgMajorVersion::GB06 => {
                // openGauss v6: assume legacy like PG14
                if info == postgres_ffi::v6::bindings::XLOG_DBASE_CREATE {
                    let createdb = XlCreateDatabase::decode(buf);
                    tracing::debug!("XLOG_DBASE_CREATE GB06");
                    ...
                    return Ok(Some(record));
                } else if info == postgres_ffi::v6::bindings::XLOG_DBASE_DROP {
                    let dropdb = XlDropDatabase::decode(buf);
