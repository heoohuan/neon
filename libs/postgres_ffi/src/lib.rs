...
/// openGauss bindings generated by bindgen (bindings_opengauss.rs)
pub mod opengauss {
    pub mod bindings {
        #![allow(dead_code)]
        #![allow(unsafe_op_in_unsafe_fn)]
        #![allow(clippy::undocumented_unsafe_blocks)]
        include!(concat!(env!("OUT_DIR"), "/bindings_opengauss.rs"));
    }
}
    ...
    impl WalStreamDecoder {
        pub fn new(lsn: Lsn, pg_version: PgMajorVersion) -> WalStreamDecoder {
            WalStreamDecoder {
                lsn,
                pg_version,
                // default: assume Postgres compatibility unless overridden
                wal_format: WalFormat::Postgres,
                inputbuf: BytesMut::new(),
                state: State::WaitingForRecord,
            }
        }

        /// Set WAL format for this decoder to choose correct CRC/page header handling.
        pub fn set_wal_format(&mut self, fmt: WalFormat) {
            self.wal_format = fmt;
        }
